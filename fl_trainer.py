...
def apply_noise_with_consensus(tensor, diff_tensor, net_freq, client_id, normal_avg_update, clip_threshold=0.3):

    norm = torch.norm(tensor)
    if norm > clip_threshold:
        tensor = tensor * (clip_threshold / norm)
    print(colored(norm, "blue"))

    consensus_direction = normal_avg_update / torch.norm(normal_avg_update)  
    adapt_factor = torch.abs(diff_tensor)
    noise = consensus_direction * adapt_factor
    return tensor + noise
...
def get_results_filename_png(poison_type, attack_method, model_replacement, project_frequency, aggregator, defense_method, norm_bound, prox_attack, attack_freq, fixed_pool=False, model_arch="vgg9"):
    current_datetime = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = "{}_{}_{}_{}".format(poison_type, model_arch, attack_method, attack_freq)
    ...
    if fixed_pool:
        ...
        filename += "_fixed_freq"
    
    if prox_attack:
        filename += "_prox_attack"
    if aggregator in ("fedavg", "fednova", "fedrobust"):
        filename += "_{}".format(aggregator)
    ...
    filename += "_{}_acc_results.png".format(current_datetime)

    return filename

def get_results_filename(poison_type, attack_method, model_replacement, project_frequency, aggregator, defense_method, norm_bound, prox_attack, attack_freq, model_arch="vgg9"):
    current_datetime = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = "{}_{}_{}_{}".format(poison_type, model_arch, attack_method, attack_freq)
    ...
    filename += "_{}_acc_results.csv".format(current_datetime)
...
def nofca_aggregator(init_model, net_list, net_freq, device, model="lenet", acc_clean=0):
    weight_accumulator = {}
    global_avg_update = {}
    normal_avg_update = {}
    ...
    for name, params in init_model.state_dict().items():
        weight_accumulator[name] = torch.zeros_like(params).float()
        global_avg_update[name] = torch.zeros_like(params).float()
        normal_avg_update[name] = torch.zeros_like(params).float()
    ...
    for i in range(num_clients):
        ...
        for name, data in net_list[i].state_dict().items():
            diff[name] = (data - init_model.state_dict()[name])
            global_avg_update[name] += diff[name] / num_clients

        total_norm = sum(torch.norm(diff[name]) for name in diff)
        client_norms.append((i, total_norm))  

    global_norm = sum(torch.norm(global_avg_update[name]) for name in global_avg_update)
    client_norms.sort(key=lambda x: abs(x[1] - global_norm), reverse=True)

    num_outliers = math.ceil(0.2 * num_clients)
    outlier_clients = [client_norms[i][0] for i in range(num_outliers)]
    normal_clients = [i for i in range(num_clients) if i not in outlier_clients]

    print(colored(f"Skipping clients {outlier_clients} due to extreme deviation from global norm.", "red"))

    num_normals = len(normal_clients)
    if num_normals > 0:
        for name, params in init_model.state_dict().items():
            normal_avg_update[name] = torch.zeros_like(params).float()
        ...
        for i in normal_clients:
            for name, data in net_list[i].state_dict().items():
                diff = data - init_model.state_dict()[name]
                normal_avg_update[name] += diff / num_normals

    for i in range(num_clients):
        if i in outlier_clients:
            continue
        ...
        for name, data in net_list[i].state_dict().items():
            ...
            print(f"\nStart performing apply_noise_with_consensus noise...")
            diff[name] = apply_noise_with_consensus(diff[name], diff[name], net_freq, i, normal_avg_update[name])
...
def test_updated_wo_print(args, device, atkmodel, model, target_transform, 
         train_loader, test_loader, epoch, trainepoch, clip_image, 
         testoptimizer=None, log_prefix='Internal', epochs_per_test=3, 
         dataset="cifar10", criterion=None, subpath_saved="", test_eps=None, 
         is_poison=False, ident="global", atk_baseline=False):
...
class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, lira_args=None, *args, **kwargs):
        ...
        self.attack_freq = arguments['attack_freq']
    ...
    def run(self, wandb_ins):
        ...
        for flr in range(1, self.fl_round+1):
            ...
            if self.defense_technique == "nofca":
                self.net_avg = nofca_aggregator(self.net_avg, net_list, net_freq, device=self.device, model=self.model, acc_clean = acc_clean)
            elif fed_aggregator == "fedavg":
            ...
            cur_training_eps_list.append(cur_training_eps)
        
        target_length = max(len(fl_iter_list), len(main_task_acc), len(backdoor_task_acc), 
            len(raw_task_acc), len(adv_norm_diff_list), len(wg_norm_list), 
            len(cur_training_eps_list))

        fl_iter_list = pad_list(fl_iter_list, target_length)
        main_task_acc = pad_list(main_task_acc, target_length)
        backdoor_task_acc = pad_list(backdoor_task_acc, target_length)
        raw_task_acc = pad_list(raw_task_acc, target_length)
        adv_norm_diff_list = pad_list(adv_norm_diff_list, target_length)
        wg_norm_list = pad_list(wg_norm_list, target_length)
        cur_training_eps_list = pad_list(cur_training_eps_list, target_length)
        ...
        df = pd.DataFrame({'fl_iter': fl_iter_list, 
                            ...
                            'cur_training_eps': cur_training_eps_list
                            })
        ...
        results_filename = f'/iba/saved_result/{get_results_filename(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.aggregator, self.defense_technique, self.norm_bound, self.prox_attack, self.attack_freq, fixed_pool=False, model_arch=self.model)}'
        df.to_csv(results_filename, index=False)
        logger.info("Wrote accuracy results to: {}".format(results_filename))

        #FL Round에 따른 acc 그래프
        plt.figure(figsize=(12, 8))

        # Main Task Accuracy
        plt.plot(df['fl_iter'], df['main_task_acc'], marker='o', linestyle='-', color='b', label='Main Task Accuracy')

        # Backdoor Accuracy
        plt.plot(df['fl_iter'], df['backdoor_acc'], marker='x', linestyle='--', color='r', label='Backdoor Accuracy')
        ...
        graph_filename = f'/iba/saved_graph/{get_results_filename_png(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.aggregator, self.defense_technique, self.norm_bound, self.prox_attack, self.attack_freq, fixed_pool=False, model_arch=self.model)}'
        plt.savefig(graph_filename)
        logger.info("Wrote accuracy graph to: {}".format(graph_filename))
...
class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    ...
    def run(self, wandb_ins):
        ...
        target_length = max(len(fl_iter_list), len(main_task_acc), len(backdoor_task_acc), 
            len(raw_task_acc), len(adv_norm_diff_list))
        ...
        results_filename = f'/iba/saved_result/{get_results_filename(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.aggregator, self.defense_technique, self.norm_bound, self.prox_attack, None, fixed_pool=True, model_arch=self.model)}'
        df.to_csv(results_filename, index=False)
        logger.info("Wrote accuracy results to: {}".format(results_filename))
        ...
        graph_filename = f'/iba/saved_graph/{get_results_filename_png(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.aggregator, self.defense_technique, self.norm_bound, self.prox_attack, None, fixed_pool=True, model_arch=self.model)}'
        plt.savefig(graph_filename)
        logger.info("Wrote accuracy graph to: {}".format(graph_filename))